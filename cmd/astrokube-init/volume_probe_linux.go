...
const (
	suidReportEnv = "ASTROKUBE_SUID_REPORT" // child prints its euid
...
func runVolumeProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== rootless volume-mount hardening probe =====")
	fmt.Println("astrokube-init: a setuid binary on a nosuid volume must NOT grant root")
	...
	if err := syscall.Mount("tmpfs", "/run/suidvol", "tmpfs", 0, ""); err != nil {
		fmt.Printf("astrokube-init: volume probe setup failed (suid tmpfs): %v\n", err)
		fmt.Println("astrokube-init: ===== end volume-mount hardening probe =====")
		return
	}
	...
	if nosuidErr != nil || noexecErr != nil {
		fmt.Printf("astrokube-init: volume probe setup failed (nosuid=%v noexec=%v)\n", nosuidErr, noexecErr)
		fmt.Println("astrokube-init: ===== end volume-mount hardening probe =====")
		return
	}

	if err := copyExe("/run/suidvol/bin"); err != nil {
		fmt.Printf("astrokube-init: copy to suidvol failed: %v\n", err)
		return
	}
	if err := copyExe("/run/nosuidvol/bin"); err != nil {
		fmt.Printf("astrokube-init: copy to nosuidvol failed: %v\n", err)
		return
	}
	if err := copyExe("/run/noexecvol/bin"); err != nil {
		fmt.Printf("astrokube-init: copy to noexecvol failed: %v\n", err)
		return
	}
	...
	if controlOK && nosuidOK && noexecDenied {
		fmt.Println("astrokube-init: volume hardening ENFORCED — nosuid blocks setuid escalation, noexec blocks exec ✓")
	} else {
		fmt.Println("astrokube-init: volume hardening INCOMPLETE — see above")
	}
	fmt.Println("astrokube-init: ===== end volume-mount hardening probe =====")
}
