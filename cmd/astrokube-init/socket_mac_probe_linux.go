...
const (
	socketMacProbeEnv = "ASTROKUBE_SOCKET_MAC_PROBE" // "main" | "peer"

	// "KUIP": label an IPv4 endpoint (arg2 = ip be-u32) with a tenant (arg3).
	prAstrokubeLabelIp = 0x4b554950
...
func labelIP(ip uintptr, tenant uintptr) error {
	if _, _, e := syscall.Syscall6(syscall.SYS_PRCTL, prAstrokubeLabelIp, ip, tenant, 0, 0, 0); e != 0 {
		return e
	}
	return nil
}
...
func runSocketMacProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== astromac network MAC probe =====")
	fmt.Println("astrokube-init: tenant-labeled IP; cross-tenant connects are mediated")
	...
	if ok {
		fmt.Println("astrokube-init: astromac network MAC is ENFORCED (cross-tenant deny + same/unconfined allow + permissive log) ✓")
	} else {
		fmt.Println("astrokube-init: astromac network MAC probe INCOMPLETE — see above")
	}
	fmt.Println("astrokube-init: ===== end astromac network MAC probe =====")
}
