...
// userns_probe verifies astrokube user-namespace Stage 1: clone(CLONE_NEWUSER)
...
const (
	usernsProbeEnv  = "ASTROKUBE_USERNS_PROBE" // "child"
	usernsParentEnv = "ASTROKUBE_USERNS_PARENT"
)
...
func runUsernsProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== user-namespace (Stage 1) probe =====")
	fmt.Println("astrokube-init: clone(CLONE_NEWUSER) should create a real child user namespace")
	...
	switch {
	case err == nil:
		fmt.Println("astrokube-init: user namespaces Stage 1 OK — clone(CLONE_NEWUSER) creates a distinct, tracked namespace ✓")
		fmt.Println("astrokube-init: (id mapping + ns-aware capabilities are Stage 2/3 — see USER-NAMESPACES.md)")
	default:
		fmt.Printf("astrokube-init: user-namespace Stage 1 INCOMPLETE — clone(CLONE_NEWUSER) failed: %v\n", err)
	}
	fmt.Println("astrokube-init: ===== end user-namespace probe =====")
}
