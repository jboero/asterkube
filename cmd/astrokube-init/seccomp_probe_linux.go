...
const (
	seccompProbeEnv = "ASTROKUBE_SECCOMP_PROBE" // "errno" | "kill"
...
func runSeccompProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== seccomp-bpf enforcement probe =====")
	...
	if errnoOK && killed {
		fmt.Println("astrokube-init: seccomp-bpf is ENFORCED (errno + kill verified) ✓")
	} else {
		fmt.Println("astrokube-init: seccomp-bpf enforcement INCOMPLETE — see failures above")
	}
	fmt.Println("astrokube-init: ===== end seccomp-bpf probe =====")
}
