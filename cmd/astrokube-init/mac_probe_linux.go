...
const (
	macProbeEnv = "ASTROKUBE_MAC_PROBE" // "main" | "peer"

	prAstrokubeSetTenant = 0x4b55544e // "KUTN"
	prAstrokubeMacMode   = 0x4b554d4d // "KUMM"
...
func prctlSetTenant(tenant uintptr) error {
	if _, _, e := syscall.Syscall6(syscall.SYS_PRCTL, prAstrokubeSetTenant, tenant, 0, 0, 0, 0); e != 0 {
		return e
	}
	return nil
}
...
func prctlMacMode(mode uintptr) error {
	if _, _, e := syscall.Syscall6(syscall.SYS_PRCTL, prAstrokubeMacMode, mode, 0, 0, 0, 0); e != 0 {
		return e
	}
	return nil
}
...
func runMacProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== astromac (native MAC) probe =====")
	fmt.Println("astrokube-init: tenant-labeled processes; cross-tenant signals are mediated")
	...
	if ok {
		fmt.Println("astrokube-init: astromac MAC is ENFORCED (cross-tenant deny + same-tenant allow + permissive log) ✓")
	} else {
		fmt.Println("astrokube-init: astromac MAC probe INCOMPLETE — see above")
	}
	...
	fmt.Println("astrokube-init: ===== end astromac probe =====")
}
