...
const sandboxProbeEnv = "ASTROKUBE_SANDBOX_NETNS" // "hold" | "join"
...
func sandboxProbeJoin() {
	pid := os.Getenv("ASTROKUBE_SANDBOX_PID")
...
func runSandboxNetnsProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== pod-sandbox netns-join probe =====")
	fmt.Println("astrokube-init: a workload joins the sandbox's network namespace via setns")
	...
	if err := sandbox.Start(); err != nil {
		fmt.Printf("astrokube-init: sandbox start failed: %v\n", err)
		fmt.Println("astrokube-init: ===== end pod-sandbox netns-join probe =====")
		return
	}
	...
	joiner.Env = append(os.Environ(), sandboxProbeEnv+"=join", fmt.Sprintf("ASTROKUBE_SANDBOX_PID=%d", sandboxPid))
	...
	if joined && distinctFromHost {
		fmt.Println("astrokube-init: pod-sandbox networking WORKS — workload joined the sandbox's netns (shared) ✓")
		fmt.Println("astrokube-init: (the pure-Go runtime now setns-joins net/ipc/uts for path-based OCI ns specs)")
	} else {
		fmt.Println("astrokube-init: pod-sandbox netns-join INCOMPLETE — see above")
	}
	fmt.Println("astrokube-init: ===== end pod-sandbox netns-join probe =====")
}
