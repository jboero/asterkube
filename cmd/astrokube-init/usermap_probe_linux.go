...
const usermapProbeEnv = "ASTROKUBE_USERMAP_PROBE" // "unpriv" | "child"
...
func runUsermapProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== rootless id-mapping (Stage 2) probe =====")
	fmt.Println("astrokube-init: unprivileged maps its uid/gid -> container sees itself as root 0")
	...
	if unpriv.ProcessState != nil && unpriv.ProcessState.ExitCode() == 0 && uerr == nil {
		fmt.Println("astrokube-init: rootless id mapping WORKS — unprivileged container root maps to host uid, sees uid 0 ✓")
	} else {
		fmt.Println("astrokube-init: rootless id-mapping probe INCOMPLETE — see above")
	}
	fmt.Println("astrokube-init: ===== end rootless id-mapping probe =====")
}
