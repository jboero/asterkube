...
const (
	fileMacProbeEnv  = "ASTROKUBE_FILE_MAC_PROBE" // "main" | "peer"
	fileMacPathEnv   = "ASTROKUBE_FILE_MAC_PATH"
	secretPath       = "/tmp/astromac-secret"

	// "KUFL": label the file at fd (arg2) with a tenant (arg3).
	prAstrokubeLabelFd = 0x4b55464c
)

// labelFd labels the open file referred to by fd with the given tenant.
func labelFd(fd uintptr, tenant uintptr) error {
	if _, _, e := syscall.Syscall6(syscall.SYS_PRCTL, prAstrokubeLabelFd, fd, tenant, 0, 0, 0); e != 0 {
		return e
	}
	return nil
}
...
func runFileMacProbe() {
	fmt.Println()
	fmt.Println("astrokube-init: ===== astromac file-access MAC probe =====")
	fmt.Println("astrokube-init: tenant-labeled file; cross-tenant reads are mediated")
	...
	if ok {
		fmt.Println("astrokube-init: astromac file MAC is ENFORCED (cross-tenant deny + same/unconfined allow + permissive log) ✓")
	} else {
		fmt.Println("astrokube-init: astromac file MAC probe INCOMPLETE — see above")
	}
	fmt.Println("astrokube-init: ===== end astromac file-access MAC probe =====")
}
