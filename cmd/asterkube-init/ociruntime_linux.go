...
const defaultRuntimeRoot = "/run/asterkube-runc"
...
func runOCIRuntime(args []string) int {
	...
	for i < len(args) {
		...
		case a == "--version", a == "-v", a == "version":
			...
			return 0
		case a == "--help", a == "-h":
			fmt.Println("asterkube pure-Go OCI runtime (runc-compatible subset)")
			return 0
...
func printRuncVersion() {
	fmt.Println("runc version 1.4.3-asterkube (pure-Go, CGO-free)")
...
func runcLog(g runtimeGlobals, format string, a ...interface{}) {
	...
	fmt.Fprintf(os.Stderr, "asterkube-runc: %s\n", msg)
	if g.log != "" {
...
func runcInit(root, id, bundle string) int {
	spec, err := readSpec(bundle)
	if err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: spec: %v\n", err)
		return 1
	}
	...
	if err := joinNamespaces(spec); err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: join ns: %v\n", err)
		return 1
	}
	rootfs := spec.Root.Path
	if !filepath.IsAbs(rootfs) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: open fifo: %v\n", err)
		return 1
	}
	...
	if err := setupRootfs(rootfs, spec); err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: rootfs: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: reopen fifo: %v\n", err)
		return 1
	}
	if _, err := wf.Write([]byte{0}); err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: signal fifo: %v\n", err)
		return 1
	}
	wf.Close()
	...
	if spec.Process == nil || len(spec.Process.Args) == 0 {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: empty process args\n")
		return 1
	}
	if spec.Process.Cwd != "" {
	...
	if err := syscall.Exec(path, spec.Process.Args, spec.Process.Env); err != nil {
		fmt.Fprintf(os.Stderr, "asterkube-runc init: exec %s: %v\n", path, err)
		return 1
	}
...
func setupRootfs(rootfs string, spec *ociSpec) error {
	...
	for _, m := range spec.Mounts {
		...
		if err := syscall.Mount(src, dest, m.Type, flags, data); err != nil {
			...
			fmt.Fprintf(os.Stderr, "asterkube-runc init: mount %s(%s)->%s: %v\n", m.Type, src, m.Destination, err)
		}
	}
