...
func apiserverPhase(kubeletBin, sock, kubeconfig string, env []string) {
	...
	if _, err := os.Stat(kubeconfig); err != nil {
		...
		fmt.Println("apiserver: stage one with asterkube/make-node-kubeconfig.sh to enable this phase")
		return
	}
	...
	// each boot (asterkube/fresh-ext2.sh) because this node is ephemeral. A
	...
	args := []string{
		...
		"--register-with-taints=asterkube.io/experimental=true:NoSchedule",
