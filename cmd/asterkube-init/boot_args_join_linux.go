...
// Boot-args cluster join: bind a GENERIC asterkube node image to a specific
...
// gives it PATH/USER), so the cluster coordinates arrive as ASTERKUBE_* env vars.
//
// Set them in OSDK.toml `kcmd_args` (baked) or, for a direct kernel boot, in
// `-append`:
//
//	ASTERKUBE_APISERVER=https://10.0.2.2:6443
//	ASTERKUBE_TOKEN=abcdef.0123456789abcdef        # kubeadm bootstrap token
//	ASTERKUBE_CA_HASH=sha256:1b2c...               # kubeadm-style CA pubkey hash
//	ASTERKUBE_NODE_NAME=web-1                       # optional (default: asterkube)
//	ASTERKUBE_CLUSTER_DNS=10.96.0.10               # optional
//	ASTERKUBE_TAINTS=key=val:NoSchedule            # optional
//	ASTERKUBE_NODE_IP=10.0.2.15                    # optional (default: eth0 slirp IP)
//	ASTERKUBE_INSECURE=1                            # skip apiserver TLS verify (dev/slirp)
//
...
const (
	envApiserver  = "ASTERKUBE_APISERVER"
	envToken      = "ASTERKUBE_TOKEN"
	envCAHash     = "ASTERKUBE_CA_HASH"
	envNodeName   = "ASTERKUBE_NODE_NAME"
	envClusterDNS = "ASTERKUBE_CLUSTER_DNS"
	envTaints     = "ASTERKUBE_TAINTS"
	envNodeIP     = "ASTERKUBE_NODE_IP"
...
func bootArgsJoin() {
	...
	node := envOr(envNodeName, "asterkube")
	nodeIP := envOr(envNodeIP, "10.0.2.15")

	fmt.Println()
	fmt.Println("asterkube-init: ===== boot-args cluster join =====")
	fmt.Printf("asterkube-init: apiserver=%s node=%q (from kernel cmdline)\n", apiserver, node)
	if token == "" {
		fmt.Printf("asterkube-init: SKIPPED join (no %s on the kernel cmdline)\n", envToken)
		return
	}
	...
	if err := writeBootstrapKubeconfig(bootstrap, apiserver, token); err != nil {
		fmt.Printf("asterkube-init: FAILED to build bootstrap kubeconfig: %v\n", err)
		return
	}
	...
	fmt.Printf("asterkube-init: kubelet TLS-bootstrapping with the token and registering node %q...\n", node)
...
// (secure) unless ASTERKUBE_INSECURE is set (dev/slirp).
func writeBootstrapKubeconfig(path, apiserver, token string) error {
	...
	switch {
	case insecure:
		fmt.Println("asterkube-init: bootstrap trust: insecure-skip-tls-verify (dev/slirp)")
	...
	case caHash != "":
		...
		fmt.Printf("asterkube-init: bootstrap trust: cluster CA verified against %s ✓\n", envCAHash)
...
func startStaticContainerd(root, state, sock, logPath string, env []string) (*exec.Cmd, bool) {
	...
	if err := cd.Start(); err != nil {
		fmt.Printf("asterkube-init: FAILED to start containerd: %v\n", err)
		return nil, false
	}
	for i := 0; i < 100; i++ {
	...
	fmt.Println("asterkube-init: containerd socket never appeared")
	dumpTail(logPath, 30)
	_ = cd.Process.Kill()
	return nil, false
}
