...
func ociSelfTest() {
	fmt.Println("asterkube-runc: self-test — running a container through the pure-Go OCI runtime (zero C)")
	...
	if err := os.MkdirAll(rootfs+"/proc", 0o755); err != nil {
		fmt.Printf("asterkube-runc: self-test FAILED mkdir: %v\n", err)
		return
	}
	...
	if err != nil {
		fmt.Printf("asterkube-runc: self-test FAILED reading %s: %v\n", exe, err)
		return
	}
	if err := os.WriteFile(rootfs+"/init", data, 0o755); err != nil {
		fmt.Printf("asterkube-runc: self-test FAILED writing rootfs/init: %v\n", err)
		return
	}
  ...
  "process": {
    "args": ["/init"],
    "env": ["ASTERKUBE_RUNC_PAYLOAD=1", "PATH=/"],
    "cwd": "/"
  },
  "root": { "path": "rootfs" },
  "hostname": "octest",
  ...
  "linux": {
    ...
    "cgroupsPath": "/asterkube-octest"
  }
}`
	if err := os.WriteFile(bundle+"/config.json", []byte(config), 0o644); err != nil {
		fmt.Printf("asterkube-runc: self-test FAILED writing config.json: %v\n", err)
		return
	}

	if rc := runOCIRuntime([]string{"--root", root, "create", "--bundle", bundle, "--pid-file", bundle + "/pid", "octest"}); rc != 0 {
		fmt.Println("asterkube-runc: self-test FAILED at create")
		return
	}
	fmt.Println("asterkube-runc: created (container init forked into fresh namespaces, blocked on exec.fifo)")
	if rc := runOCIRuntime([]string{"--root", root, "start", "octest"}); rc != 0 {
		fmt.Println("asterkube-runc: self-test FAILED at start")
		return
	}
	time.Sleep(1 * time.Second)
	fmt.Print("asterkube-runc: state -> ")
	runOCIRuntime([]string{"--root", root, "state", "octest"})
	runOCIRuntime([]string{"--root", root, "delete", "--force", "octest"})
	fmt.Println("asterkube-runc: self-test complete")
}
