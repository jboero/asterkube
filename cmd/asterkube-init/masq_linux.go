...
const (
	// prAsterkubeMasq is the prctl option that marks a bridge as a masquerade
	// uplink (arg2 = the bridge's interface index).
	prAsterkubeMasq = 0x4b55424d
...
const (
	masqRoleEnv   = "ASTERKUBE_MASQ_ROLE"
	masqTargetEnv = "ASTERKUBE_MASQ_TARGET"
	// masqUplinkGwEnv passes the uplink gateway address the server expects to
	// see as the (masqueraded) source.
	masqUplinkGwEnv = "ASTERKUBE_MASQ_UPLINK_GW"
)
...
func addMasqUplink(idx int) error {
	_, _, errno := syscall.Syscall6(prSysPrctl, prAsterkubeMasq, uintptr(idx), 0, 0, 0, 0)
	if errno != 0 {
		return errno
	}
	return nil
...
// containerMasqTest runs inside a bridged pod, gated on ASTERKUBE_MASQ_ROLE.
