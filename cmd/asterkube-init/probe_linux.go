...
const probeChildEnv = "ASTERKUBE_PROBE_CHILD"
...
const probeBindPortEnv = "ASTERKUBE_BIND_PORT"
...
const (
	probeMemCgroupEnv = "ASTERKUBE_MEM_CGROUP"
	probeMemMBEnv     = "ASTERKUBE_MEM_MB"
)
...
const (
	probeCPUCgroupEnv = "ASTERKUBE_CPU_CGROUP"
	probeCPUMsEnv     = "ASTERKUBE_CPU_MS"
)
...
func probeKernel() {
	fmt.Println()
	fmt.Println("asterkube-init: kernel capability probe (scoping Kubernetes support)")
...
func cpuEnforcementTest() string {
	const (
		cgroup  = "/sys/fs/cgroup/asterkube.cputest"
		burnMs  = 1000
...
func memEnforcementTest() string {
	const (
		cgroup   = "/sys/fs/cgroup/asterkube.memtest"
		limitMiB = 32
		allocMiB = 128
	)
	if err := os.Mkdir(cgroup, 0o755); err != nil {
