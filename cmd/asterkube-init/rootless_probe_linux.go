...
const (
	rootlessProbeEnv = "ASTERKUBE_ROOTLESS_PROBE" // "unpriv" | "userns"
...
func runRootlessProbe() {
	fmt.Println()
	fmt.Println("asterkube-init: ===== rootless (user-namespace capability) probe =====")
	fmt.Println("asterkube-init: unprivileged -> userns gains caps in its OWN ns, stays powerless on host")
	...
	if unpriv.ProcessState != nil && unpriv.ProcessState.ExitCode() == 0 && uerr == nil {
		fmt.Println("asterkube-init: rootless is ENFORCED — unprivileged gains caps inside its userns, denied on host ✓")
	} else {
		fmt.Println("asterkube-init: rootless probe INCOMPLETE/FAILED — see above (boundary not verified)")
	}
	fmt.Println("asterkube-init: ===== end rootless probe =====")
}
