...
func setupCNI(cniSrcDir string) {
	...
	if err := os.WriteFile("/etc/cni/net.d/10-asterkube.conflist", []byte(cniConflist), 0o644); err != nil {
		fmt.Printf("cni: FAILED to write conflist: %v\n", err)
		return
	}
	fmt.Printf("cni: wrote /etc/cni/net.d/10-asterkube.conflist (ptp + portmap, %s)\n", podCIDR)
}
