...
const containerEnv = "ASTERKUBE_CONTAINER"

// podHostnameEnv passes a pod's hostname to its container.
const podHostnameEnv = "ASTERKUBE_POD_HOSTNAME"
...
const manifestDir = "/etc/asterkube/pods"

// Environment variables passing a veth pod's addresses to its container.
const (
	podIPEnv     = "ASTERKUBE_POD_IP"
	hostIPEnv    = "ASTERKUBE_HOST_IP"
	vethPrefxEnv = "ASTERKUBE_VETH_PREFIX"
)
...
const (
	podTenantEnv     = "ASTERKUBE_POD_TENANT"
	tenantProbeIPEnv = "ASTERKUBE_TENANT_PROBE_IP"
)
...
const crossPingEnv = "ASTERKUBE_CROSS_PING"
...
// asterkube end-to-end milestone — the node actually running pods using the
// PID/UTS/IPC/NET namespaces and cgroup limits implemented in the kernel.
func runNodeAgent() {
	fmt.Println()
	fmt.Println("asterkube-init: node agent starting (kubelet static-pod mode)")
	...
	if pureGoMode() {
		fmt.Println("asterkube-init: skipping the kube-proxy ClusterIP demo — kube-proxy")
		fmt.Println("asterkube-init: does not run on the zero-C image; the bridged-pod Service")
		fmt.Println("asterkube-init: DNAT below already exercises the kernel NAT + LB path.")
	} else {
	...
	fmt.Println("asterkube-init: node agent: all pods completed")
}
...
func startPodContainer(spec podSpec, netEnv []string) *podHandle {
	fmt.Printf("asterkube-init: starting pod %q (mem=%dMiB cpu=%d%%, namespaces=pid,uts,ipc,net,mnt)\n",
...
func waitPod(spec podSpec, h *podHandle) {
	...
	if waitErr != nil {
		fmt.Printf("asterkube-init: pod %q exited with error: %v\n", spec.Name, waitErr)
	} else {
		fmt.Printf("asterkube-init: pod %q completed successfully\n", spec.Name)
	}
}
...
func runBridgedPods(specs []podSpec) {
	...
	for _, spec := range specs {
		...
		if !ok {
			fmt.Printf("asterkube-init: pod %q skipped: bridge %q FAILED\n", spec.Name, spec.Network.Bridge)
			continue
		}
		wg.Add(1)
...
// connectivity, all driven by real netlink rather than any asterkube-specific
// shim.
