...
const envDHCPRenewSec = "ASTERKUBE_DHCP_RENEW_SEC"
...
func dhcpFirst(iface string) bool {
	fmt.Println()
	fmt.Printf("asterkube-init: -- DHCP: bringing up %s before anything else (cloud-init style) --\n", iface)

	nl, err := nlOpen()
	if err != nil {
		fmt.Printf("asterkube-init: DHCP SKIPPED (netlink: %v)\n", err)
		return false
	}
	idx, mac, err := nl.linkByName(iface)
	nl.close()
	if err != nil {
		fmt.Printf("asterkube-init: DHCP SKIPPED (no %s: %v)\n", iface, err)
		return false
	}
	if mac == nil {
		...
		fmt.Printf("asterkube-init: DHCP: NIC MAC not exposed by kernel; using fallback %s\n", mac)
	}

	xid := uint32(time.Now().UnixNano())
	lease, err := dhcpAcquire(mac, xid, 4*time.Second)
	if err != nil {
		fmt.Printf("asterkube-init: DHCP failed (%v) — falling back to static config\n", err)
		return false
	}
	...
	nl2, err := nlOpen()
	if err != nil {
		fmt.Printf("asterkube-init: DHCP got a lease but netlink reopen failed: %v\n", err)
		return false
	}
	defer nl2.close()
	if err := nl2.addAddrV4(idx, lease.ip, prefix); err != nil {
		fmt.Printf("asterkube-init: DHCP: assign %s/%d failed: %v\n", lease.ip, prefix, err)
		return false
	}
	if lease.gateway != nil && !lease.gateway.IsUnspecified() {
		if err := nl2.addDefaultRouteV4(lease.gateway, idx); err != nil {
			fmt.Printf("asterkube-init: DHCP: default route via %s failed: %v\n", lease.gateway, err)
		}
	}
	...
	fmt.Printf("asterkube-init: DHCP OK — %s/%d via %s, dns=%s, lease=%ds (from %s) ✓\n",
...
// the lease is lost. Set ASTERKUBE_DHCP_RENEW_SEC to override T1 (for testing).
func dhcpRenewLoop(mac net.HardwareAddr, idx int, lease *dhcpLease) {
	...
	if nl, e := dhcpRenew(mac, lease, false); e == nil {
		lease = nl
		fmt.Printf("asterkube-init: DHCP renewal verified — RENEWING ACKed (lease=%ds) ✓\n", lease.leaseSec)
	} else if nl, e := dhcpRenew(mac, lease, true); e == nil {
		lease = nl
		fmt.Printf("asterkube-init: DHCP renewal verified — REBINDING ACKed (lease=%ds) ✓\n", lease.leaseSec)
	}
	...
	for {
		...
		if nl, e := dhcpRenew(mac, lease, false); e == nil {
			...
			fmt.Printf("asterkube-init: DHCP renewed (RENEWING) — lease=%ds via %s ✓\n", lease.leaseSec, lease.serverID)
			continue
		}
		...
		if nl, e := dhcpRenew(mac, lease, true); e == nil {
			lease = nl
			applyLeaseDelta(idx, lease)
			fmt.Printf("asterkube-init: DHCP renewed (REBINDING) — lease=%ds ✓\n", lease.leaseSec)
			continue
		}
		fmt.Println("asterkube-init: DHCP renew+rebind failed; re-DISCOVERing a fresh lease")
