...
const (
	...
	prSysPrctl = 157
	// prAsterkubeDNAT is the prctl option that programs a Service DNAT rule.
	prAsterkubeDNAT = 0x4b554244
)
...
const (
	svcVIPEnv         = "ASTERKUBE_SVC_VIP"
	svcVPortEnv       = "ASTERKUBE_SVC_VPORT"
	svcBackendPortEnv = "ASTERKUBE_SVC_BACKEND_PORT"
	svcRoleEnv        = "ASTERKUBE_SVC_ROLE"
)
...
func addServiceDNAT(vip string, vport int, backend string, bport int, proto int) error {
	...
	_, _, errno := syscall.Syscall6(prSysPrctl, prAsterkubeDNAT,
...
// ASTERKUBE_SVC_ROLE being set. Everything is bounded — a missing/late peer
