#!/bin/bash
# astrokube-join.sh — generate a "join bundle" config drive that binds a generic
# astrokube node image to YOUR Kubernetes cluster, the way cloud-init/Ignition
...
#   -drive if=none,format=raw,id=join,file=astrokube-join.img \
#   -device virtio-blk-pci,drive=join,serial=astrokubecfg
# The node looks for a disk whose serial/label is "astrokubecfg", mounts it, and
# joins. (run-host-qemu.sh: JOIN_BUNDLE=astrokube-join.img ./run-host-qemu.sh astrokube)
#
# Usage: astrokube/astrokube-join.sh [-n node-name] [-s apiserver-host[:port]]
#                                    [-a apiserver-ip] [-d cluster-dns]
#                                    [-o out.img] [--dir DIR]
set -euo pipefail

NODE="astrokube-$(hostname -s 2>/dev/null || echo node)"
...
OUT="astrokube-join.img"
DIRONLY=""              # if set, emit a plain directory instead of a disk image
TAINTS="astrokube.io/experimental=true:NoSchedule"
...
WORK=$(mktemp -d /tmp/astrokube-join.XXXXXX)
...
: > "$BUNDLE/.astrokube-join"   # marker so the node recognizes this disk
...
if [ -n "$DIRONLY" ]; then
  ...
  echo "==> wrote join bundle dir: $DIRONLY  (share it via virtio-fs, tag=astrokubecfg)"
else
  command -v mke2fs >/dev/null || { echo "!! mke2fs (e2fsprogs) not found; use --dir instead" >&2; exit 1; }
  # 16 MiB ext2, populated rootlessly from the bundle dir; label astrokubecfg.
  rm -f "$OUT"; truncate -s 16M "$OUT"
  mke2fs -q -t ext2 -L astrokubecfg -d "$BUNDLE" "$OUT" >/dev/null
  echo "==> wrote join bundle image: $OUT ($(ls -lh "$OUT" | awk '{print $5}'), ext2, label=astrokubecfg)"
  echo "    attach it to the VM:"
  echo "      JOIN_BUNDLE=$OUT ./run-host-qemu.sh astrokube"
  echo "    or manually:"
  echo "      -drive if=none,format=raw,id=join,file=$OUT -device virtio-blk-pci,drive=join,serial=astrokubecfg"
fi
