#!/bin/bash
# build-qcow2.sh — produce a QCOW2 of the self-contained asterkube node image.
#
...
# to ride along.
#
# Boot it with:  ./run-host-qemu.sh asterkube-qcow2
#
# Usage: asterkube/build-qcow2.sh
set -euo pipefail
...
QCOW=${QCOW:-test/initramfs/build/asterkube-node.qcow2}

[ -f "$ISO" ] || { echo "!! missing $ISO — build it first (asterkube/zero-c-initramfs.sh)" >&2; exit 1; }
...
echo "    boot it: ./run-host-qemu.sh asterkube-qcow2"
