#!/bin/bash
# asterkube-boot-args.sh — print the kernel boot args that bind a generic
# asterkube node image to YOUR cluster, kubeadm-style. Run it on a box with
...
#   4. prints ASTERKUBE_* boot args for both OSDK.toml `kcmd_args` and `-append`.
#
...
#
# Usage: asterkube/asterkube-boot-args.sh [-n node-name] [-s apiserver-host:port]
...
ARGS=( "ASTERKUBE_APISERVER=${GUEST_SERVER}" "ASTERKUBE_TOKEN=${TOKEN}" "ASTERKUBE_NODE_NAME=${NODE}" )
[ -n "$CLUSTER_DNS" ] && ARGS+=( "ASTERKUBE_CLUSTER_DNS=${CLUSTER_DNS}" )
if [ -n "$SECURE" ] && [ -n "$CA_HASH" ]; then
  ARGS+=( "ASTERKUBE_CA_HASH=${CA_HASH}" )
else
