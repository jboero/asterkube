...
# re-building the asterkube-init Go binary and swapping it into the EXISTING
...
#
# Usage: asterkube/rebuild-initramfs.sh [path-to-kubelet-repo]
set -euo pipefail
cd "$(dirname "$0")/../asterinas"                       # asterinas/
KUBELET=${1:-$(cd "$(dirname "$0")/.." && pwd)}
BUILD=test/initramfs/build
CPIO="$BUILD/initramfs.cpio.gz"
WORK=$(mktemp -d /tmp/asterkube-initramfs.XXXXXX)
trap 'rm -rf "$WORK"' EXIT

echo "==> building asterkube-init (static, CGO disabled) from $KUBELET"
