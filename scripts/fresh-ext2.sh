...
# ext2 filesystem. The astrokube node is ephemeral and uses the ext2 disk for
...
#
# Usage: astrokube/fresh-ext2.sh [size-mb]   (default 256)
set -euo pipefail
...
RAW=$(mktemp /tmp/astrokube-ext2.XXXXXX.img)
trap 'rm -f "$RAW"' EXIT
