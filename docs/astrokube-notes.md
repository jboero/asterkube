# astrokube — a Kubernetes node with no C and no libc
...
separate, out-of-tree static Go binary (referred to here as `astrokube-init`) and
is not part of this repository.

## Status: the node runs pods ✅

The Go init now acts as a **node agent**: it reads static pod manifests
(Kubernetes `--pod-manifest-path` style) from `/etc/astrokube/pods/*.json` and runs
...
```
astrokube-init: node agent starting (static-pod mode)
astrokube-init: starting pod "hello" (mem=128MiB cpu=50%, namespaces=pid,uts,ipc,net,mnt)
...
astrokube-init: pod "hello" completed successfully
astrokube-init: starting pod "web" (mem=64MiB cpu=25%, …)
...
```
            ...
            │        ▼                                       │
            │   /sbin/init ──symlink──▶ astrokube-init       │
            │        │ (static CGO_ENABLED=0 Go binary)      │
            │        ▼                                       │
            │   astrokube-init: getpid()==1 ?               │
...
```
 astrokube-init: running as PID 1 on the Asterinas kernel
 Rust kernel + Go init, no C, no libc.

astrokube-init: mounting filesystems
  [ ok ] /proc                (proc)
  [ ok ] /sys                 (sysfs)
...
astrokube-init: milestone 1 complete (filesystems mounted). Powering off.
```
...
Build the out-of-tree `astrokube-init` Go program as a static binary, then stage a
minimal initramfs around it (no busybox, no C/libc):

```bash
# Build your astrokube-init Go binary (static, CGO disabled) to /tmp/astrokube-init,
# e.g. CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o /tmp/astrokube-init <pkg>

# Minimal initramfs: the init binary + a /sbin/init symlink to it.
ROOT=/tmp/astrokube-initramfs
mkdir -p "$ROOT"/{usr/bin,sbin,proc,sys,dev,tmp,run,ext2,exfat}
cp /tmp/astrokube-init "$ROOT/usr/bin/astrokube-init"
ln -sf ../usr/bin/astrokube-init "$ROOT/sbin/init"
...
```bash
docker run -d --name astrokube --privileged --network=host -v /dev:/dev \
    -v "$PWD/asterinas:/root/asterinas" -w /root/asterinas \
    asterinas/asterinas:0.18.0-20260603 sleep infinity

docker exec astrokube bash -lc 'OSDK_LOCAL_DEV=1 cargo install cargo-osdk --path osdk'
docker exec astrokube bash astrokube-run.sh      # creates virtio disk images, then cargo osdk run
```
...
[`../OSDK.toml`](../OSDK.toml) `[run.boot]` is set for astrokube:

- `init=/sbin/init` (the symlink to `astrokube-init`), `console=hvc0`, empty
  `init_args` (no shell — astrokube ships no busybox).
